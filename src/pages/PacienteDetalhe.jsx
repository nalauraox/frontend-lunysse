                ...
                <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-dark mb-2">
                            ...
                            <select 
                            value={data.time}
                            onChange= {() => onChange({...data, time: e.target.value})}
                            className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-dark/50 focus:outline-none focus:rin-2 focus:ring-light"
                            required
                            >
                                <option value="">Selecione um horário</option>
                                {timeSlots.map(time => <option key={time} value={time}>{value}
                                </option>)} 
                            </select>
                    </div>
                </div>
                <Input 
                    ...
                    <div>
                        <label className="block text-sm font-medium text-dark mb-2">
                            ...
                            <select 
                            value={data.duration}
                            onChange= {() => onChange({...data, duration: parseInt(e.target.value) })}
                            className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-dark/50 focus:outline-none focus:rin-2 focus:ring-light"
                            required
                            >
                                <option value="">Selecione a duração da sessão</option>
                                {durations.map(duration => <option key={duration} value={duration}>{duration} Minutos
                                </option>)} 
                            </select>
                    </div>
                    <div className="flex gap-4 ">
                        ...
                        <Button type="submit"loaading={loading} className="flex-1" disable=
                        {!data.date || date.time}>Agendar Sessão</Button>
                    </div>
            </form>
        </Card>
    )
